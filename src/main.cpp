...
int64 CTransaction::nMinTxFee = 1000000; //2000000;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 100000; //2000000;
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && (23*nFees) < txMinFee)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    bnNew /= nTargetTimespan; // the smaller nTargetTimespan is, the larger is bnNew increase
...
bool VerifyDB() {
    ...
    int nCheckDepth = GetArg( "-checkblocks", 60); // 288
    if (nCheckDepth == 0)
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0000") << OP_CHECKSIG;
        CBlock block;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "getblocks")
    {
        ...
        // reduce nLimit in order to reduce processing time when receiving a getblocks request
